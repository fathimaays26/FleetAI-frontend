...
export default function VinRankedList({
  title, description, vins, selectedVin, onSelectVin, totalMatching, getPrimaryStat,
}) {
  const [query, setQuery] = useState("");
  const [visibleCount, setVisibleCount] = useState(10);

  const filtered = vins.filter(
    (v) => v.vin.toLowerCase().includes(query.toLowerCase()) || v.model.toLowerCase().includes(query.toLowerCase())
  );

  const riskColor = { red: "text-red-600", amber: "text-amber-600", green: "text-emerald-600" };

  return (
    <div className="w-[380px] shrink-0 border-r border-gray-100 pr-5">
      <div className="flex items-center gap-2 mb-1">
        <span className="w-5 h-5 rounded bg-purple-50 text-purple-500 text-xs font-medium flex items-center justify-center">01</span>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-left transition-colors ${
                active ? "bg-purple-50 border border-purple-200" : "hover:bg-gray-50 border border-transparent"
              }`}
            >
              <div>
                <div className="text-sm font-medium text-gray-900">{v.vin}</div>
                <div className="text-xs text-gray-400">{v.model} · {v.parts.length} part{v.parts.length > 1 ? "s" : ""} tracked</div>
              </div>
              <span className={`text-sm font-semibold ${riskColor[v.riskTier]}`}>{getPrimaryStat(v)}</span>
            </button>
          );
        })}
      </div>
